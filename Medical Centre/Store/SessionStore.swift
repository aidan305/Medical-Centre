...
class SessionStore: ObservableObject {
        ...
        handle = Auth.auth().addStateDidChangeListener({ (auth, user) in
            if let user = user {
                if self.newSignUp == true {
                    ...
                    self.session = User(uid: user.uid, email: user.email, name: self.name, newUser: true, phoneNumber: self.phoneNumber, DOB: self.dateOfBirth)
                    if let user = self.session{
                        self.repo.registerUserIntoDatabase(user: user) {
    ...
    func signUp(email: String, password: String, name: String, phoneNumber: String, DOB: Date, handler: @escaping AuthDataResultCallback) {
